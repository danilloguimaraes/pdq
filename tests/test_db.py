...
INSERT INTO attendance VALUES (1, 1, 'J', 'obs');
PRAGMA user_version = 2;
"""


def test_migrates_v2_database_adding_payment(tmp_path):
    path = tmp_path / "pdq.db"
    raw = sqlite3.connect(path)
    ...
    assert conn.execute("SELECT status, note FROM attendance").fetchone()[:] == ("J", "obs")
    conn.execute(
